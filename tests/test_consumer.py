...
class TestConsumerHelpers(unittest.TestCase):
    def test_retry_sleep_seconds_uses_exponential_backoff(self):
        with patch.object(consumer_module.random, "uniform", return_value=0):
            self.assertEqual(consumer_module._retry_sleep_seconds(1), consumer_module.RETRY_BASE_DELAY_SECONDS)
            self.assertEqual(consumer_module._retry_sleep_seconds(2), consumer_module.RETRY_BASE_DELAY_SECONDS * 2)
...
class TestDatabaseFunctions(unittest.TestCase):
    def test_load_csv_to_postgres_copies_data_and_commits(self):
        ...
        with tempfile.NamedTemporaryFile("w+", encoding="utf-8", newline="", delete=False) as csv_file:
        ...
        try:
            consumer_module.load_csv_to_postgres(csv_path, conn, "pipeline.table")
        finally:
            os.unlink(csv_path)
    ...
    def test_setup_database_executes_schema_table_and_migrations(self):
        ...
        consumer_module.setup_database(conn)
        ...
        for migration in consumer_module.RAW_USER_OPERATION_MIGRATIONS:
    ...
    def test_insert_event_to_db_inserts_ordered_values(self):
        conn = MagicMock()
        cursor = conn.cursor.return_value
        event = {event_key: f"value-{event_key}" for _, event_key in consumer_module.RAW_USER_OPERATION_FIELDS}

        consumer_module.insert_event_to_db(conn, event)

        cursor.execute.assert_called_once()
        args, _ = cursor.execute.call_args
        query, values = args
        self.assertIn("INSERT INTO pipeline.raw_user_operations", query)
        self.assertEqual(values, tuple(event[event_key] for _, event_key in consumer_module.RAW_USER_OPERATION_FIELDS))
        conn.commit.assert_called_once()
        conn.rollback.assert_not_called()
...
class TestConsumerClass(unittest.TestCase):
    def test_get_db_connection_creates_new_connection_when_missing(self):
        with patch.object(consumer_module, "connect_to_postgres", return_value=MagicMock(closed=False)) as connect_mock:
            worker = consumer_module.Consumer(postgres_params={"host": "localhost"})
    ...
    def test_get_db_connection_reuses_existing_connection(self):
        ...
        with patch.object(consumer_module, "connect_to_postgres") as connect_mock:
            conn = worker.get_db_connection()
    ...
    def test_callback_acknowledges_message_on_success(self):
        ...
        with patch.object(worker, "get_db_connection", return_value=worker.db_conn), patch.object(
            consumer_module, "insert_event_to_db"
        ) as insert_mock:
    ...
    def test_callback_nacks_and_resets_connection_on_processing_error(self):
        ...
        with patch.object(worker, "get_db_connection", return_value=db_conn), patch.object(
            consumer_module, "insert_event_to_db", side_effect=RuntimeError("db down")
        ):
            worker.callback(channel, method, None, body)
